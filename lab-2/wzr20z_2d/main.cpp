...
void VirtualWorldCycle()
{
	...
	for (map<int, MovableObject*>::iterator it = other_vehicles.begin(); it != other_vehicles.end(); ++it)
	{
		...
		if (veh)
		{
			...
			Vector3 w_rotation = veh->state.vV_ang * avg_cycle_time + veh->state.vA_ang * avg_cycle_time * avg_cycle_time / 2;

			// nowy kwaternion obrotu - zgodnie z wzorem z instrukcji -(wektor obrotu znormalizowany, dlugosc wektora obrotu)
			quaternion q_rotation = AsixToQuat(w_rotation.znorm(), w_rotation.length());

			// nowy kwaternion zastepuje stary
			veh->state.qOrient = q_rotation * veh->state.qOrient;
