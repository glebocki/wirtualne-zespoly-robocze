...
MovableObject::MovableObject(Terrain *t)             // konstruktor                   
{
	...
	mass_own = 1000.0;     // masa w³asna obiektu [kg] (bez paliwa)
	...
	{
		state.vPos = Vector3(210.024780, 11.272700, -257.841095);
		state.qOrient = quaternion(0.000000, 0.960792, 0.000000, -0.277247);
	}
...
void Terrain::DrawObject()
{
	...
	GLfloat GroundSurfaceColor[] = { 0.60f, 0.47f, 0.1f, 1.0f };
				...
				for (long w = 0; w < loczek_nr_rozdz; w++)
					...
					{						
						...
						if (sek->map_of_heights_in_edit)
							...
							glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, GroundSurfaceColor);
