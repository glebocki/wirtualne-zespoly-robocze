...
void InteractionInitialisation()
{
	...
	multi_reciv = new multicast_net("224.10.12.75", 10001);      // Object do odbioru ramek sieciowych
	multi_send = new multicast_net("224.10.12.75", 10001);       // Object do wysy³ania ramek
	...
	int       nCmdShow)
{
	...
	main_window = CreateWindow(class_name, "WZR 2020/21, temat 4, wersja b [Y-autopilot, F11-test autop.]", WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
		50, 50, 1400, 1000, NULL, NULL, hInstance, NULL);
...
void MessagesHandling(UINT message_type, WPARAM wParam, LPARAM lParam)
{
	...
	{                    // lub w widoku z góry przesuwanie mapy (Ctrl+Home - powrót do poł. bazowego)
		...
		{
			map_shift_mode = true;
			/*if (par_view.tracking){
				par_view.shift_to_bottom += my_vehicle->state.vPos.z;
				par_view.shift_to_right += my_vehicle->state.vPos.x;
				par_view.tempor_tracking = true;
				par_view.tracking = false;
			}
			else par_view.tempor_tracking = false;*/
