...
MovableObject::MovableObject(Terrain *t)             // konstruktor                   
{
	...
	mass_own = 700.0;     // masa w³asna obiektu [kg] (bez paliwa)
	...
	state.vPos = Vector3(220, clearance + height / 2 + 10, -20);
	...
	quaternion qObr = AsixToQuat(Vector3(0, 1, 0), 80 * PI / 180.0);
	...
	fuel_collection_skills = 0.1 + (float)(rand() % 10) / 10;
	money_collection_skills = 0.1 + (float)(rand() % 10) / 10;
...
void MovableObject::DrawObject()
{
	...
	glPrint("uzm=%.1f_uzp=%.1f", this->money_collection_skills, this->fuel_collection_skills);
	glPopMatrix();
}
